# Fina — Swift / SwiftUI Project Status
...
- [x] Scaffold project directory structure (`Fina`, `App`, `Models`, `Services`, `ViewModels`, `Views`, `Components`)
- [x] Setup Data Models (`Models.swift` for `Transaction`, `Category`, `UserSettings`)
- [x] Setup Theme (`AppTheme.swift` with custom colors)
- [x] Setup App Entry Point (`FinaApp.swift`)
...
- [ ] Create a new macOS App project in Xcode (`Fina`).
- [ ] Drag and drop the `Fina/App`, `Models`, `Services`, `ViewModels`, `Views`, and `Components` folders into the Xcode project.
